...
class NodoTreap {
    ...
    public NodoTreap(int clave) {
        this.clave = clave;
        this.prioridad = new Random().nextInt(100); // Prioridad aleatoria
    }
}
...
class Treap {
    ...
    private NodoTreap insertar(NodoTreap nodo, int clave) {
        if (nodo == null) return new NodoTreap(clave);

        if (clave < nodo.clave) {
            nodo.izquierdo = insertar(nodo.izquierdo, clave);
            if (nodo.izquierdo.prioridad > nodo.prioridad) nodo = rotarDerecha(nodo);
        } else if (clave > nodo.clave) {
            nodo.derecho = insertar(nodo.derecho, clave);
            if (nodo.derecho.prioridad > nodo.prioridad) nodo = rotarIzquierda(nodo);
        }
        return nodo;
    }
...
public class Test_Treap {
    public static void main(String[] args) {
        ...
        System.out.println("Treap después de inserciones:");
        treap.imprimir();

        treap.eliminar(30);
        System.out.println("Treap después de eliminar 30:");
        treap.imprimir();
    }
}
